...
class RecipeTests(APITestCase):
    ...
    def test_create_recipe_unauthenticated(self):
        ...
        response = self.client.post(reverse('recipe:recipe-create'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        
    def test_retrieve_recipe(self):
        response = self.client.get(reverse('recipe:recipe-detail', kwargs={'pk': self.recipe.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_recipe_unauthenticated(self):
        ...
        response = self.client.put(reverse('recipe:recipe-detail', kwargs={'pk': self.recipe.id}), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        
    def test_delete_recipe_success(self):
        response = self.client.delete(reverse('recipe:recipe-detail', kwargs={'pk': self.recipe.id}))
    ...
    def test_like_recipe_success(self):
        response = self.client.post(reverse('recipe:recipe-like', kwargs={'pk': self.recipe.id}))
    ...
    def test_unlike_recipe_success(self):
        ...
        response = self.client.delete(reverse('recipe:recipe-like', kwargs={'pk': self.recipe.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
